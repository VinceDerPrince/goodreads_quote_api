...
def popular_quotes(page: int) -> List[str]:
    ...
    for i in range(len(quotes)):
        ...
        if "tags:" in clean_text:
            output[i] = {
                "quote": clean_text[clean_text.index("“"):clean_text.index("”")+1],
                "name": clean_text[clean_text.index("”")+4:clean_text.index("tags:")-1],
                "tags": clean_text[clean_text.index("tags:")+5:clean_text.index(re.findall("[0-9]+",clean_text)[0])].split(),
                "likes": re.findall("[0-9]+",clean_text)[0]
            }
        else:
            output[i] = {
                "quote": clean_text[clean_text.index("“"):clean_text.index("”")+1],
                "name": clean_text[clean_text.index("”")+4:clean_text.index(re.findall("[0-9]+",clean_text)[0])],
                "tags": "no tags",
                "likes": re.findall("[0-9]+",clean_text)[0]
            }
    return output